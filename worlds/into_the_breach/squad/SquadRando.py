...
def shuffle_teams(random: Random, filtered_squad_names: list[str]) -> dict[str, Squad]:
    ...
    def get_id_from_names(unit_name: str, squad_name: str) -> int:
        return get_id_from_ids(unit_ids[unit_name], squad_ids[squad_name])

    def get_names_from_id(solver_id: int) -> Tuple[str, str]:
    ...
    for unit_id in range(1, unit_count + 1):
        for squad_id_1 in range(squad_count - 1):
            for squad_id_2 in range(squad_id_1 + 1, squad_count):
                solver.add_clause([-get_id_from_ids(unit_id, squad_id_1), -get_id_from_ids(unit_id, squad_id_2)])
    ...
    for squad_id in range(squad_count):
        all_unit_ids = [get_id_from_ids(unit_id, squad_id) for unit_id in range(1, unit_count + 1)]
        for unit_id_1 in range(1, unit_count):
            incomplete_clause = all_unit_ids.copy()
            incomplete_clause.remove(get_id_from_ids(unit_id_1, squad_id))
            for unit_id_2 in range(unit_id_1 + 1, unit_count + 1):
                ...
                for unit_name_4 in units_per_class["Science"]:
                    for squad_name in filtered_squad_names:
                        solver.add_clause([-get_id_from_names(unit_name_1, squad_name),
                                           ...
                                           -get_id_from_names(unit_name_4, squad_name)])
    ...
    for solver_id in assumptions:
        if solver_id > 0:
            unit_name, squad_name = get_names_from_id(solver_id)
